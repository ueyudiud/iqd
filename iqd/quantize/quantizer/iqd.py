...
from ..module import MaskedNodeQ
...
class IQDQuantizer(QuantizerMixin):
    ...
    def __init__(self,
        ...
        assert weight_schedule in ('1', 'snr', 'max-snr-1', 'min-snr-5', 'diff-snr'), ValueError(f"Unsupported weight schedule: {weight_schedule}")

        def gather_stage(kwargs) -> Stage:
            kwargs2 = {
                'lr': kwargs.pop('lr', lr),
                'lr_decay': kwargs.pop('lr_decay', lr_decay),
            }
            kwargs2.update(kwargs)
            return Stage(**kwargs2)

        self.stages = list(map(gather_stage, stages))
    ...
    def _wmap(self, method, grouping, calibration):
        def apply(value: Tensor, /):
            ...
            if self._train:
                return MaskedLeafQ(function, value)
            else:
    ...
    @torch.no_grad()
    def _incremental_quantize(self,
                              target: Target,
                              wpr: float,
                              apr: float
                              ):
        for module in target.activations:
            module.rate = apr

        if wpr == 0.0:
            return PruningState(quantized_weight_rate=0.0, quantized_activation_rate=apr)

        num_total_weight = 0
        num_quant_weight = 0

        for origin, module in target.weights:
            weight = module.value
            # weight_shape = weight.shape

            n = weight.numel()
            n_q = int(n * wpr)
            # pr_sqrt = sqrt(pruning_rate)
            # k = int(weight.numel() * (1.0 - pr_sqrt))
            if n_q == 0:
                continue
            k = n - n_q

            value = module.function.encode(origin)

            if k > 0:
                e = torch.where(module.mask, float('inf'), origin.abs())
                e_t = e.flatten().kthvalue(k).values
                mask = e >= e_t
                mask = mask.logical_and_(module.mask.logical_not())
                module.value.masked_scatter_(mask, value[mask])
                module.mask.logical_or_(mask)
            else:
                module.value.copy_(value)
                module.mask.fill_(True)

            num_total_weight += weight.numel()
            num_quant_weight += module.mask.count_nonzero().item()

        return PruningState(
            quantized_weight_rate=num_quant_weight / num_total_weight if num_total_weight != 0 else 0.0,
            quantized_activation_rate=apr
        )
    ...
    def _calibrate(self,
        ...
        try:
            ...
            for batched_samples in sample_loader:
                ...
                kwargs = self.sampler.sample_kwargs(len(batched_samples))
    ...
    def _quantize_aware_train(self,
        ...
        for batched_samples in data_loader:
            batched_samples = batched_samples.to(target_unet.device)
            kwargs = self.sampler.sample_kwargs(len(batched_samples))

            timesteps = self.scheduler.timesteps[torch.randperm(self.num_timestep, generator=generator)]

            for timestep in timesteps:
                batched_epsilon = self.sampler.sample_noise(batched_samples, generator)
                batched_noised_samples = self.sampler.q_sample(batched_samples, batched_epsilon, timestep)
                alpha_cumprod = self.scheduler.alphas_cumprod[timestep]
                snr = (alpha_cumprod / (1 - alpha_cumprod)).sqrt()
                ...
                match self.config.weight_schedule:
                    ...
                    case 'snr':
                        loss_scale = snr
                    case 'max-snr-1':
                        loss_scale = max(snr, 1) / snr
                    case 'min-snr-5':
                        loss_scale = min(snr, 5) / snr
                    case 'diff-snr':
                        next_alpha_cumprod = self.scheduler.alphas_cumprod[timestep - 1] if timestep > 0 else self.scheduler.final_alpha_cumprod
                        next_snr = (next_alpha_cumprod / (1 - next_alpha_cumprod)).sqrt()
                        loss_scale = 1 / snr - 1 / next_snr
                    case _:
                        raise ValueError
    ...
    def quantize(self,
        ...
        optimizer = torch.optim.Adam(target.parameters, lr=self.config.lr)
        ...
        with tqdm(total=progress_scale * total_epoch, unit='iteration') as progress:
            ...
            for stage in self.stages:
                pruned_state = self._incremental_quantize(target, wpr=stage.wpr, apr=stage.apr)
                state = {
                    'wpr': pruned_state.quantized_weight_rate,
                    'apr': pruned_state.quantized_activation_rate,
                }
                num_stage += 1
                num_step = 0
                if callback is not None:
